...
//    获取所有投票
    ...
    public PollResponse getPollById(Long pollId, UserPrincipal currentUser){
        ...
        if (currentUser != null){
            userVote = (Vote) voteRepository.findByUserIdAndPollId(currentUser.getId(), pollId);
        }
    ...
    private void validatePageNumberAndSize(int page, int size) {
        ...
        if (size > AppConstants.MAX_PAGE_SIZE){
            throw new BadRequestException("Page number cannot be greater than" + AppConstants.MAX_PAGE_SIZE);
        }
    }
}
